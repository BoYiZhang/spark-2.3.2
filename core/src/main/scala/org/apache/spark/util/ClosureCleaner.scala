...
private[spark] object ClosureCleaner extends Logging {
  ...
  /**
    *辅助方法区清理指定的闭包
    *ClosureCleaner类的作用就是递归清理外围类中无用域，降低序列化的开销，防止不必要的不可序列化异常。
    * 降低网络io,提高executor的内存效率
    *
    *
    * 其机制是遍历封闭闭包的层次结构，并将开始闭包实际上没有使用但仍然包含在编译的匿名类中的引用空出。
    * 请注意，简单地更改封闭闭包是不安全的，因为其他代码路径可能依赖于它们。
    * 相反，我们克隆每个封闭闭包并相应地设置父指针。
    *
    * 默认情况下，闭包是临时清理的。这意味着我们可以检测封闭的对象是否确实被起始对象引用(直接引用或传递引用)，
    * 如果没有，则从层次结构中断开这些闭包。
    * 换句话说，除了空出未使用的字段引用之外，我们还空出引用起始闭包实际上不需要的封闭对象的任何父指针。
    * 我们通过跟踪最终由内部闭包调用的所有方法的树来确定传递性，并记录流程中引用的所有字段。
    *
    * 例如，在以下场景中需要传递清理：
    ...
    *   }
    *
    * 在此示例中，范围“two”不可序列化，因为它引用范围“one”，它引用SomethingNotSerializable。
    * 但请注意，范围“two”的主体实际上并不依赖于SomethingNotSerializable。
    * 这意味着我们可以安全地将克隆范围“one”的父指针清空，并将其设置为范围“2”的父级，
    * 这样范围“two”不再传递SomethingNotSerializable。
    * func  开始闭包清理
    * checkSerializable  在清理之后，是否验证是否可以序列化
    * cleanTransitively 是否开启关联清理
    * accessedFields  将一个映射从一个类转换为一组由起始闭包访问的字段
    *
    *
    *
    *
  ...
  private def clean(
      ...
      accessedFields: Map[Class[_], Set[String]]): Unit = {
    logInfo("递归清理外围类中无用域，降低序列化的开销，防止不必要的不可序列化异常。  降低网络io,提高executor的内存效率")
