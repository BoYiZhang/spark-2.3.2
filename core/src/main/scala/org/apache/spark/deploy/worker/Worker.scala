...
private[deploy] class Worker(
  ...
  private def sendRegisterMessageToMaster(masterEndpoint: RpcEndpointRef): Unit = {
    ...
    logInfo(" worker 向master发送注册信息 ");
    logInfo(" 调用信息里面的方法是： Master.scala 中的receive 方法中的 ==> RegisterWorker ");
  ...
  override def receive: PartialFunction[Any, Unit] = synchronized {
    ...
    case LaunchExecutor(masterUrl, appId, execId, appDesc, cores_, memory_) =>
      if (masterUrl != activeMasterUrl) {
        ...
        try {
          ...
          // todo ExecutorRunner  仅仅 standalone 模式在用
    ...
    case LaunchDriver(driverId, driverDesc) =>
      ...
      //todo 仅仅  standalone 模式使用 DriverRunner
...
private[deploy] object Worker extends Logging {
  ...
  def main(argStrings: Array[String]) {
    ...
    //输入值
    //  host = "192.168.103.188"
    //  port = 0
    //  webUiPort = 8081
    //  cores = 4
    //  memory = 15360
    //  masters = {String[1]@1419}
    //  0 = "spark://sysadmindeMacBook-Pro.local:7077"
    //  workDir = null
    //  propertiesFile = null
  ...
  def startRpcEnvAndEndpoint(
    ...
    logInfo("获取master节点地址")
    val masterAddresses = masterUrls.map(RpcAddress.fromSparkURL(_))

    logInfo("设置rpc环境参数  执行 new Worker 方法")
