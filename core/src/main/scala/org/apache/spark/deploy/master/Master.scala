...
private[deploy] class Master(
  ...
  // spark 恢复模式四种   ZOOKEEPER     FILESYSTEM    CUSTOM     NONE
  ...
  override def onStart(): Unit = {
    ...
    // 选择持久化方式 ZOOKEEPER FILESYSTEM  CUSTOM 默认不做持久化
    val (persistenceEngine_, leaderElectionAgent_) = RECOVERY_MODE match {
      ...
      case "CUSTOM" =>
        ...
        logInfo("MonarchyLeaderAgent 这里会调用选取leader方法 ")
  ...
  override def receive: PartialFunction[Any, Unit] = {
    ...
    case ElectedLeader =>
      ...
      state = if (storedApps.isEmpty && storedDrivers.isEmpty && storedWorkers.isEmpty) {
        //todo  如果没有任何持久化信息, 直接将 Master 的当前状态设置为激活.
        RecoveryState.ALIVE
      } else {

        //todo  如果有持久化信息, 就直接设置会恢复模式, 下面会对任务进行恢复 .
      ...
      if (state == RecoveryState.RECOVERING) {
        ...
        //todo 开始恢复集群  处理  app  drvier  worker
        beginRecovery(storedApps, storedDrivers, storedWorkers)

        //todo 开始恢复 , 向 master 节点发送信息, 进行集群恢复.   CompleteRecovery
    ...
    case RegisterWorker(
      ...
      logInfo(" 这里是注册worker的方法，由worker主动调用这个方法 ");


      logInfo("Registering worker %s:%d with %d cores, %s RAM".format(
        workerHost, workerPort, cores, Utils.megabytesToString(memory)))

      logInfo(" 当前master的节点状态如果为standby状态，直接向worker发送信息 MasterInStandby  ");
      if (state == RecoveryState.STANDBY) {
        // work节点不做任何处理，直接忽略
        workerRef.send(MasterInStandby)

        logInfo(" 如果worker节点已经被加载过，则忽略 ");
      } else if (idToWorker.contains(id)) {
        workerRef.send(RegisterWorkerFailed("Duplicate worker ID"))
      } else {
        logInfo(" 创建workerInfo的一个信息对象 ");
        val worker = new WorkerInfo(id, workerHost, workerPort, cores, memory,
          workerRef, workerWebUiUrl)
        logInfo(" 对workerInfo对象注册 registerWorker ");
    ...
    case RegisterApplication(description, driver) =>
      ...
      if (state == RecoveryState.STANDBY) {
        ...
        //todo   org.apache.spark.deploy.client.StandaloneAppClient
  ...
  private def scheduleExecutorsOnWorkers(
    ...
    def canLaunchExecutor(pos: Int): Boolean = {
      logInfo("判断指定worker能否为app启动executor")

      logInfo("判断集群是否还有cpu可以分配一个executors所需要的cpu嘛")
      val keepScheduling = coresToAssign >= minCoresPerExecutor

      logInfo("当前worker还有足够的cpu去分配一个executors所需要的cpu嘛")
      val enoughCores = usableWorkers(pos).coresFree - assignedCores(pos) >= minCoresPerExecutor

      // If we allow multiple executors per worker, then we can always launch new executors.
      // Otherwise, if there is already an executor on this worker, just give it more cores.

      logInfo("当前worker是否已经启动了Executor。。。")
      val launchingNewExecutor = !oneExecutorPerWorker || assignedExecutors(pos) == 0
      if (launchingNewExecutor) {
        logInfo("已经分配的内存 = 已经分配的executors的数量* 每个executors的内存数量")
        val assignedMemory = assignedExecutors(pos) * memoryPerExecutor
        logInfo("足够的内存 = 已使用的内存-已分配的内存 >= 还需在分配一个executor的内存数量 ")
        val enoughMemory = usableWorkers(pos).memoryFree - assignedMemory >= memoryPerExecutor
        logInfo("是否已经达到app分配executor的数量限制 ")
        ...
        logInfo("我们已经添加了cores给一个已经存在的executor，内存大小已经分配过了，所以只需要检查cpu数量是否足够就好了。")
    ...
    logInfo("找出可以启动的worker节点")
    var freeWorkers = (0 until numUsable).filter(canLaunchExecutor)


    logInfo("在worker上启动分配资源")
    while (freeWorkers.nonEmpty) {
      freeWorkers.foreach { pos =>
        ...
        while (keepScheduling && canLaunchExecutor(pos)) {
          ...
          logInfo("spreadOutApps 这个是个参数，可以控制executor分配的数量   spark.deploy.spreadOut 默认true ")
          logInfo("默认 ：尽可能的把executors分配到更多的worker节点上")
          logInfo("另一种：在一个worker上分配更多的资源，当worker上的资源占用满了之后，在去下一个worker上分配")
          if (spreadOutApps) {
  ...
  private def startExecutorsOnWorkers(): Unit = {
    logInfo("startExecutorsOnWorkers:Schedule and launch executors on workers")
    // Right now this is a very simple FIFO scheduler. We keep trying to fit in the first app
    // in the queue, then the second app, etc.
    logInfo("循环遍历waitingApps ，为其分配资源。。")
    ...
    for (app <- waitingApps) {
      ...
      if (app.coresLeft >= coresPerExecutor) {
        ...
        logInfo("获取到所有可以用的worker，内存、cpu 并按cpu倒序排列 ")
        val usableWorkers = workers.toArray.filter(_.state == WorkerState.ALIVE)
          .filter(worker => worker.memoryFree >= app.desc.memoryPerExecutorMB &&
            worker.coresFree >= coresPerExecutor)
          .sortBy(_.coresFree).reverse

        logInfo("该方法实现了在worker集群中分配executor的策略")
  ...
  private def allocateWorkerResourceToExecutors(
    ...
    logInfo("worker 启动一个或多个executors" )
    logInfo("一种：executor 上启动的cpu数量已经确定，将会把cpu平均分配到没有剩余 " )
    logInfo("二种：启动一个executor 将cpu全部给他 " )

    // If the number of cores per executor is specified, we divide the cores assigned
    // to this worker evenly among the executors with no remainder.
    // Otherwise, we launch a single executor that grabs all the assignedCores on this worker.

    logInfo("确定executor数量：numExecutors" )
    val numExecutors = coresPerExecutor.map { assignedCores / _ }.getOrElse(1)

    logInfo("确定可分配cpu：coresToAssign数量" )
    val coresToAssign = coresPerExecutor.getOrElse(assignedCores)


    for (i <- 1 to numExecutors) {
      val exec = app.addExecutor(worker, coresToAssign)
      logInfo("launchExecutor........")
   ...
   * every time a new app joins or resource availability changes.
    * 这里是调度方法，基本上对集群资源的变更，节点状态的改变，任务的提交，都会执行此调度
   */
  private def schedule(): Unit = {
    ...
    // Drivers 优先级高于executors ？

    // todo 这里将workers任务进行过滤，并用Random.shuffle将workers进行打散操作，确保资源分配的平均
    ...
    logInfo(" 分配driver任务 ");
    //以迭代的方式循环waitingDrivers
    //以循环的方式给waiting driver 分配worker
    //对于每个driver，我们从分配了driver的最后一名worker开始，并继续循环，直到我们遍历了所有alive worker。
    for (driver <- waitingDrivers.toList) { // iterate over a copy of waitingDrivers
      ...
      while (numWorkersVisited < numWorkersAlive && !launched) {
        //获取worker 从shuffledAliveWorkers下标0 开始 规则，每次+1 ，但是小于numWorkersAlive的数量
        val worker = shuffledAliveWorkers(curPos)
        numWorkersVisited += 1
        logInfo(" 获取符合资源的workder节点以启动driver ； 即：workder的内存和CPU必须大于等于driver所需要的内存和cpu ， ");
        if (worker.memoryFree >= driver.desc.mem && worker.coresFree >= driver.desc.cores) {
          logInfo(" 启动Driver ");
          launchDriver(worker, driver)
          logInfo("  waitingDrivers 去掉已经启动成功的driver ");
          waitingDrivers -= driver
          launched = true
        }
        logInfo(" curPos 自增+1但是小于numWorkersAlive的数量 ");
        curPos = (curPos + 1) % numWorkersAlive
      }
    }

    logInfo("执行 启动worker节点上的executors  ==> startExecutorsOnWorkers")
    startExecutorsOnWorkers()
  }

  private def launchExecutor(worker: WorkerInfo, exec: ExecutorDesc): Unit = {
    logInfo("向worker发动信息启动executor ： Launching executor " + exec.fullId + " on worker " + worker.id)
  ...
  private def registerWorker(worker: WorkerInfo): Boolean = {
    logInfo(" 对workers缓存中的数据进行清理，过滤掉host、port相同且 状态已经DEAD的workinfo信息 ");
    workers.filter { w =>
    ...
    logInfo(" 缓存中的worker地址进行清理，如果worker地址为UNKNOWN，移除 ");
    ...
    logInfo(" 添加worker节点，并且缓存worker的信息、地址 ");
    workers += worker    // HashSet
    idToWorker(worker.id) = worker  // 结构 HashMap[String, WorkerInfo]
    addressToWorker(workerAddress) = worker //结构 HashMap[RpcAddress, WorkerInfo
    true
  }
  ...
  private def launchDriver(worker: WorkerInfo, driver: DriverInfo) {
    ...
    worker.addDriver(driver)
    driver.worker = Some(worker)
    logInfo(" 向worker发送启动driver的请求LaunchDriver ");
...
private[deploy] object Master extends Logging {
  ...
  def main(argStrings: Array[String]) {
    ...
    //组装 参数
    val
    args = new MasterArguments(argStrings, conf)
    // 开启服务
