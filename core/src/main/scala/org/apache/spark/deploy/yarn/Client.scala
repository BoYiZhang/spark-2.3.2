...
private[spark] class Client(
  ...
  /**
   *  提交一个application 到ResourceManager ， 运行我们自定义的ApplicationMaster
   *  Yarn API 提供了 (YarnClient#createApplication) for
   *  创建applications 设置applications 提交内容
   *  但是  alpha API 不支持 .
    *
    *
    *
...
//
      //todo 先注释掉啊 等连接后在说
//      logInfo(" 启动任务 从集群获取一个NodeManagers启动application")
      yarnClient.start()
//
...
//        .format(yarnClient.getYarnClusterMetrics.getNumNodeManagers))
      ...
      logInfo(" 创建application")
      val newApp = yarnClient.createApplication()

      logInfo(" 获取application 请求响应")
      val newAppResponse = newApp.getNewApplicationResponse()
      appId = newAppResponse.getApplicationId()

      logInfo(" 构建与hadoop通讯的客户端")
      new CallerContext("CLIENT", sparkConf.get(APP_CALLER_CONTEXT),
        Option(appId.toString)).setCurrentContext()

      logInfo("  验证集群是否有足够的资源")
      verifyClusterResources(newAppResponse)

      logInfo("  设置& 创建 YarnClientApplication")
  ...
  def createApplicationSubmissionContext(
    ...
    logInfo("设置ContainerLaunchContext 。。。。。。。")
    appContext.setAMContainerSpec(containerContext)
    logInfo("设置ApplicationType ： SPARK")
    appContext.setApplicationType("SPARK")

    logInfo("设置Application标签。。。。")
    ...
    sparkConf.get(MAX_APP_ATTEMPTS) match {
      ...
      case None => logInfo(s"${MAX_APP_ATTEMPTS.key} is not set. " +
          "Cluster's default value will be used.")
    }

    logInfo("定义AM失败跟踪校验间隔。 " +
            "运行了至少要运行这么多时间后，其失败计数才被重置。这个特性只有配置其值后才会生效，且只支持Hadoop-2.6+")
    ...
    logInfo("一个YARN节点标签表达式（node label expression），以此来限制AM可以被调度到哪些节点上执行。" +
            "只有Hadoop 2.6+才能支持节点标签表达式，所以如果用其他版本运行，这个属性将被忽略。")
  ...
  private def verifyClusterResources(newAppResponse: GetNewApplicationResponse): Unit = {
    ...
    //  executorMemoryOverhead : spark.executor.memoryOverhead (默认为 executor的0.1 ，最小为384M)
    //
    //
    //
    //  amMemory ： 集群模式： spark.driver.memory
    //  非集群模式： spark.yarn.am.memory （默认512M）
    //
    //  amMemoryOverhead  ：  集群模式： spark.driver.memoryOverhead
    //  非集群模式： spark.yarn.am.memoryOverhead (默认为 amMemory 的0.1 ，最小为384M)
    ...
    println("verifyClusterResources  验证资源通过。。。。。")
  ...
  private[yarn] def copyFileToRemote(
    ...
    logInfo("加载资源文件qualifiedDestPath ： " + destFs.makeQualified(destPath))
  ...
  def prepareLocalResources(
    ...
    logInfo("Upload Spark and the application JAR to the remote file system if necessary,")
    logInfo("and add them as local resources to the application master.")
    ...
    if (credentials != null) {
      ...
      logInfo(SparkHadoopUtil.get.dumpTokens(credentials).mkString("\n"))
    }
    ...
    if (sparkArchive.isDefined) {
      ...
      sparkConf.get(SPARK_JARS) match {
        ...
        case None =>
          ...
          try {
            ...
            jarsDir.listFiles().foreach { f =>
              if (f.isFile && f.getName.toLowerCase(Locale.ROOT).endsWith(".jar") && f.canRead) {
                logInfo("将文件进行打包成zip文件 文件名称：" + f.getAbsoluteFile + f.getName  )
          ...
          logInfo("将Jar文件进行分发 。。。：  hdfs://bj-rack001-hadoop002:8020/user/hadoop/.sparkStaging/application_1544436077214_6463 ");
    ...
    logInfo("cachedSecondaryJarLinks  ..........")
    logInfo("Do the same for any additional resources passed in through ClientArguments.")
    logInfo("Each resource category is represented by a 3-tuple of:")
    logInfo("   (1) comma separated list of resources in this category,")
    logInfo("   (2) resource type, and")
    logInfo("   (3) whether to add these resources to the classpath")
    List(
  ...
  /**
   * 设置一个ContainerLaunchContext 启动  ApplicationMaster container.
   * 设置启动环境, java 变量, AM启动命令
   */
  private def createContainerLaunchContext(newAppResponse: GetNewApplicationResponse)
    ...
    logInfo("加载环境launchEnv。。。。。。")
    val launchEnv = setupLaunchEnv(appStagingDirPath, pySparkArchives)

    logInfo("加载环境localResources。。。。。。")
    val localResources = prepareLocalResources(appStagingDirPath, pySparkArchives)

    //    18/12/17 17:01:54 INFO Client: 将文件进行打包成zip文件 文件名称：/workspace/spark-2.3.2/assembly/target/scala-2.11/jars/xercesImpl-2.9.1.jarxercesImpl-2.9.1.jar
    //    18/12/17 17:01:54 INFO Client: 将文件进行打包成zip文件 文件名称：/workspace/spark-2.3.2/assembly/target/scala-2.11/jars/xmlenc-0.52.jarxmlenc-0.52.jar
    //    18/12/17 17:01:54 INFO Client: 将文件进行打包成zip文件 文件名称：/workspace/spark-2.3.2/assembly/target/scala-2.11/*.jar
    //    18/12/17 17:01:54 INFO Client: 将Jar文件进行分发 。。。：  hdfs://bj-rack001-hadoop002:8020/user/hadoop/.sparkStaging/application_1544436077214_6463
    //    18/12/17 17:02:06 INFO Client: Uploading resource file:/private/var/folders/sf/ylyw50415l59z1ggvcyyt6m40000gn/T/spark-7e72a98e-f09a-4982-b7e0-c01d42028d51/__spark_libs__6945786770822454051.zip -> hdfs://bj-rack001-hadoop002:8020/user/hadoop/.sparkStaging/application_1544436077214_6469/__spark_libs__6945786770822454051.zip
    //    18/12/17 17:02:26 INFO Client: 加载资源文件qualifiedDestPath ： hdfs://bj-rack001-hadoop002:8020/user/hadoop/.sparkStaging/application_1544436077214_6469/__spark_libs__6945786770822454051.zip
    //    18/12/17 17:02:40 INFO Client: Uploading resource file:/A/spark-test/spark-test.jar -> hdfs://bj-rack001-hadoop002:8020/user/hadoop/.sparkStaging/application_1544436077214_6469/spark-test.jar
    //    18/12/17 17:02:40 INFO Client: 加载资源文件qualifiedDestPath ： hdfs://bj-rack001-hadoop002:8020/user/hadoop/.sparkStaging/application_1544436077214_6469/spark-test.jar
    //    18/12/17 17:02:42 INFO Client: Uploading resource file:/private/var/folders/sf/ylyw50415l59z1ggvcyyt6m40000gn/T/spark-7e72a98e-f09a-4982-b7e0-c01d42028d51/__spark_conf__4274797020630529296.zip -> hdfs://bj-rack001-hadoop002:8020/user/hadoop/.sparkStaging/application_1544436077214_6469/__spark_conf__.zip
    //    18/12/17 17:02:42 INFO Client: 加载资源文件qualifiedDestPath ： hdfs://bj-rack001-hadoop002:8020/user/hadoop/.sparkStaging/application_1544436077214_6469/__spark_conf__.zip
    ...
    if (isClusterMode) {
      ...
      logInfo("DRIVER_JAVA_OPTIONS  对应参数: spark.driver.extraJavaOptions")
      sparkConf.get(DRIVER_JAVA_OPTIONS).foreach { opts =>
        javaOpts ++= Utils.splitCommandString(opts).map(YarnSparkHadoopUtil.escapeForShell)
      }

      logInfo("DRIVER_LIBRARY_PATH  对应参数: spark.driver.extraLibraryPath")
      ...
      logInfo("AM_JAVA_OPTIONS 对应参数: spark.yarn.am.extraJavaOptions")
    ...
    //  userClass = {$colon$colon@8477} "::" size = 2
    //  0 = "--class"
    //  1 = "'WordCount'"
    val userClass =
      if (isClusterMode) {
    ...
    //  userJar = {$colon$colon@8515} "::" size = 2
    //  0 = "--jar"
    //  1 = "file:/A/spark-test/spark-test.jar"
    val userJar =
    ...
    //  userArgs = {ArrayBuffer@8535} "ArrayBuffer" size = 2
    //  0 = "--arg"
    //  1 = "'/tmp/zl/data/data.txt'"
    val userArgs = args.userArgs.flatMap { arg =>
      Seq("--arg", YarnSparkHadoopUtil.escapeForShell(arg))
    }

    //  amArgs  "::" size = 9
    //  0 = "org.apache.spark.deploy.yarn.ApplicationMaster"
    //  1 = "--class"
    //  2 = "'WordCount'"
    //  3 = "--jar"
    //  4 = "file:/A/spark-test/spark-test.jar"
    //  5 = "--arg"
    //  6 = "'/tmp/zl/data/data.txt'"
    //  7 = "--properties-file"
    //  8 = "{{PWD}}/__spark_conf__/__spark_conf__.properties"
    val amArgs =
    ...
    //  5 = "org.apache.spark.deploy.yarn.ApplicationMaster"
    //  6 = "--class"
    //  7 = "'WordCount'"
    //  8 = "--jar"
    //  9 = "file:/A/spark-test/spark-test.jar"
    //  10 = "--arg"
    //  11 = "'/tmp/zl/data/data.txt'"
    //  12 = "--properties-file"
    //  13 = "{{PWD}}/__spark_conf__/__spark_conf__.properties"
    //  14 = "1>"
    //  15 = "<LOG_DIR>/stdout"
    //  16 = "2>"
    //  17 = "<LOG_DIR>/stderr"
    ...
    logInfo("    resources:")
    localResources.foreach { case (k, v) => logInfo(s"        $k -> $v")}
    logInfo("    command:")
    logInfo(s"        ${printableCommands.mkString(" ")}")
    logInfo("===============================================================================")

    //
    //  18/12/17 17:02:43 INFO Client: ===============================================================================
    //  18/12/17 17:02:43 INFO Client: YARN AM launch context:
    //  18/12/17 17:02:43 INFO Client:     user class: WordCount
    //  18/12/17 17:02:43 INFO Client:     env:
    //  18/12/17 17:02:44 INFO Client:         CLASSPATH -> {{PWD}}<CPS>{{PWD}}/__spark_conf__<CPS>{{PWD}}/__spark_libs__/*<CPS>/usr/hdp/2.6.5.0-292/hadoop/conf<CPS>/usr/hdp/2.6.5.0-292/hadoop/*<CPS>/usr/hdp/2.6.5.0-292/hadoop/lib/*<CPS>/usr/hdp/current/hadoop-hdfs-client/*<CPS>/usr/hdp/current/hadoop-hdfs-client/lib/*<CPS>/usr/hdp/current/hadoop-yarn-client/*<CPS>/usr/hdp/current/hadoop-yarn-client/lib/*<CPS>/usr/hdp/current/ext/hadoop/*<CPS>$HADOOP_MAPRED_HOME/share/hadoop/mapreduce/*<CPS>$HADOOP_MAPRED_HOME/share/hadoop/mapreduce/lib/*<CPS>{{PWD}}/__spark_conf__/__hadoop_conf__
    //  18/12/17 17:02:44 INFO Client:         SPARK_YARN_STAGING_DIR -> hdfs://bj-rack001-hadoop002:8020/user/hadoop/.sparkStaging/application_1544436077214_6469
    //  18/12/17 17:02:44 INFO Client:         SPARK_USER -> hadoop
    //  18/12/17 17:02:44 INFO Client:     resources:
    //  18/12/17 17:02:44 INFO Client:         __app__.jar -> resource { scheme: "hdfs" host: "bj-rack001-hadoop002" port: 8020 file: "/user/hadoop/.sparkStaging/application_1544436077214_6469/spark-test.jar" } size: 27358 timestamp: 1545037361201 type: FILE visibility: PRIVATE
    //  18/12/17 17:02:44 INFO Client:         __spark_libs__ -> resource { scheme: "hdfs" host: "bj-rack001-hadoop002" port: 8020 file: "/user/hadoop/.sparkStaging/application_1544436077214_6469/__spark_libs__6945786770822454051.zip" } size: 217705632 timestamp: 1545037346992 type: ARCHIVE visibility: PRIVATE
    //  18/12/17 17:02:44 INFO Client:         __spark_conf__ -> resource { scheme: "hdfs" host: "bj-rack001-hadoop002" port: 8020 file: "/user/hadoop/.sparkStaging/application_1544436077214_6469/__spark_conf__.zip" } size: 216860 timestamp: 1545037363368 type: ARCHIVE visibility: PRIVATE
    //  18/12/17 17:02:44 INFO Client:     command:
    //  18/12/17 17:02:44 INFO Client:         {{JAVA_HOME}}/bin/java -server -Xmx1024m -Djava.io.tmpdir={{PWD}}/tmp -Dspark.yarn.app.container.log.dir=<LOG_DIR> org.apache.spark.deploy.yarn.ApplicationMaster --class 'WordCount' --jar file:/A/spark-test/spark-test.jar --arg '/tmp/zl/data/data.txt' --properties-file {{PWD}}/__spark_conf__/__spark_conf__.properties 1> <LOG_DIR>/stdout 2> <LOG_DIR>/stderr
    //  18/12/17 17:02:44 INFO Client: ===============================================================================
  ...
  /**
   * 向 ResourceManager 提交一个application .
   * 如果设置spark.yarn.submit.waitAppCompletion值为true ，会一致保持alive状态
   * 汇报application的状态，直到退出。
   * 负责的话client进程会在提交之后退出
   * 如果 application 处于 failed, killed, or undefined 装填，将会抛出 SparkException  异常
   */
  def run(): Unit = {

    logInfo(" 提交application ")
    this.appId = submitApplication()


    logInfo(" fireAndForget spark.yarn.submit.waitAppCompletion 值 ")
