 ...
 *
 *
 * --name spark-test --class WordCount --master yarn --deploy-mode cluster /A/spark-test/spark-test.jar /tmp/zl/data/data.txt
 *
 *
 */
...
object SparkSubmit extends CommandLineUtils with Logging {
  ...
  override def main(args: Array[String]): Unit = {
    ...
    logInfo("在程序启动之前初始化日志")
    val uninitLog = initializeLogIfNecessary(true, silent = true)

    logInfo("传入整理参数，加载环境变量")
    val appArgs = new SparkSubmitArguments(args)

    logInfo("是否启动debug模式， 默认false ， 我把默认值给改成true了，方便看日志。。    ")
    if (appArgs.verbose) {
      // scalastyle:off println
    ...
    appArgs.action match {
      ...
      case SparkSubmitAction.SUBMIT => {
        ...
        logInfo("您好： 进入程序提交模式： SparkSubmitAction.SUBMIT  ")
        submit(appArgs, uninitLog)
      }

      case SparkSubmitAction.KILL =>{
        logInfo("您好： 进入程序杀死模式： SparkSubmitAction.KILL  ")
        kill(appArgs)
      }
      case SparkSubmitAction.REQUEST_STATUS => {
        logInfo("您好： 进入程序查看模式： SparkSubmitAction.REQUEST_STATUS  ")
  ...
  private def submit(args: SparkSubmitArguments, uninitLog: Boolean): Unit = {
    ...
    // todo childMainClass:  org.apache.spark.deploy.yarn.YarnClusterApplication

    val (childArgs, childClasspath, sparkConf, childMainClass) = prepareSubmitEnvironment(args)

    def doRunMain(): Unit = {
      //todo  代理用户 args.proxyUser 查看 SparkSubmitArguments类 由参数控制 --proxy-user 或者 --principal  控制
    ...
    // 在 standalone cluster 模式, 有两种 gateways 提交方式:
    //   (1) 传统的 RPC gateway 方式 ，  使用的是 o.a.s.deploy.Client
    //   (2) 在 Spark 1.3 以后，使用 new REST-based gateway
    // Spark 1.3之后默认使用 new REST-based gateway   , but Spark submit will fail over
    // 但是  Spark submit 提交失败的话 ， 使用的是传统的gateways .


    if (args.isStandaloneCluster && args.useRest) {
      printStream.println("是 Cluster 模式，切使用新的REST-based gateway 默认true  ")
      try {
        // scalastyle:off println
        printStream.println("Running Spark using the REST application submission protocol.")
        // scalastyle:on println

        //TODO 启动Main方法 REST-based
        doRunMain()


      } catch {

        //TODO 无法启动REST，使用传统方式提交
        case e: SubmitRestConnectionException =>
          printWarning(s"Master endpoint ${args.master} was not a REST server. " +
            "Falling back to legacy submission gateway instead.")

          //TODO 启动Main方法,使用传统RPC 模式
          args.useRest = false
          submit(args, false)
      }
  ...
  private def doPrepareSubmitEnvironment(
    ...
    if (clusterManager == YARN) {
      ...
      //todo 这个是加载yarn所需要的类 目前加载不进去 我先禁用掉，等会看看
      //todo 类名： org.apache.spark.deploy.yarn.YarnClusterApplication
      println(" 这个是加载yarn所需要的类 目前加载不进去 我先禁用掉，等会看看 类名： org.apache.spark.deploy.yarn.YarnClusterApplication ")
...
//      }
    ...
    // todo  In yarn-cluster mode, use yarn.Client as a wrapper around the user class
    if (isYarnCluster) {
   ...
   * running cluster deploy mode or python applications.
    *
    * 在提供的启动环境下运行 main class
    * 如果在集群模式下启动的话， 这个mian方法，是不是用户自己提供的main class
  ...
  private def runMain(
    ...
    printStream.println(" 获取类加载器 loader")
    val loader =
    ...
    printStream.println(" 获取类加载器 loader [ org.apache.spark.util.MutableURLClassLoader ]  : "+ loader )
    Thread.currentThread.setContextClassLoader(loader)

    printStream.println(" loader 加载类， 类格式必须为 file 或者local 否则不加载  ")
    ...
    try {
      printStream.println(" 加载mainClass ：  Class.forName(className, true, getContextOrSparkClassLoader)  ")
    ...
    printStream.println(" 由 mainClass 创建 SparkApplication ")


    //todo 由  org.apache.spark.deploy.yarn.YarnClusterApplication 实例化  创建 SparkApplication
    val app: SparkApplication = if (classOf[SparkApplication].isAssignableFrom(mainClass)) {
      ...
      printWarning("mainClass无法进行实例化，由JavaMmainApplication进行实例化")
    ...
    try {
      //TODO 关键点
      println("--------------启动 实例化的类啊！！ start --------------")
      println(" yarn 集群模式下： org.apache.spark.deploy.yarn.YarnClusterApplication   ")
      app.start(childArgs.toArray, sparkConf)
      println("--------------启动 实例化的类啊！！ end ----------------")
