...
private[spark] class Executor(
  ...
  def launchTask(context: ExecutorBackend, taskDescription: TaskDescription): Unit = {
    logInfo("executor 启动task：  ")


    logInfo("将task封装成 TaskRunner   ")
    val tr = new TaskRunner(context, taskDescription)

    logInfo("加入等待队列 runningTasks  ： new ConcurrentHashMap[Long, TaskRunner]     ")
    runningTasks.put(taskDescription.taskId, tr)


    logInfo(" 执行线程 TaskRunner    ")
  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      logInfo("创建TaskMemoryManager 内存管理器  。。。。。。。。。。。")
      ...
      logInfo("线程加载类 加载器 & 加载资源   replClassLoader    " + replClassLoader  )
      ...
      logInfo(s"更新任务状态为： RUNNING   ==》 Running $taskName (TID $taskId)")
      ...
      try {
        ...
        logInfo("必须在设置updateDependencies 调用之前调用。  已获得必要的访问属性，比如访问控制。。。")
        Executor.taskDeserializationProps.set(taskDescription.properties)

        logInfo("加载依赖资源 文件 和 jar包   。。。")
        updateDependencies(taskDescription.addedFiles, taskDescription.addedJars)


        logInfo("获取task信息：  类加载器："+ Thread.currentThread.getContextClassLoader)
