...
class DAGScheduler(
  ...
  def getCacheLocs(rdd: RDD[_]): IndexedSeq[Seq[TaskLocation]] = cacheLocs.synchronized {
    ...
    // 注意： 这个不用`getOrElse()` 因为这个方法被调用0（任务数）次
  ...
  private def getOrCreateShuffleMapStage(
    ...
    shuffleIdToMapStage.get(shuffleDep.shuffleId) match {
      case Some(stage) =>
        logInfo("  getOrCreateShuffleMapStage  从存储中已经可以拿到数据=> shuffleDep：" +  shuffleDep)
        logInfo("  这里有点迷惑人，不要以为不用到，是所有的已创建过的  ShuffleMapStage  直接返回。递归的时候有调用。。。  ");
        stage

      case None =>

        logInfo(" 深度遍历获取所有祖先宽依赖，按照祖先->子辈的顺序 处理宽依赖, ")
        ...
        deps.foreach { dep =>
          ...
          if (!shuffleIdToMapStage.contains(dep.shuffleId)) {
            logInfo("将所有的未被缓存的stage提交创建ShuffleMapStage。（没有缓存就是没有被处理过）")
  ...
  def createShuffleMapStage(shuffleDep: ShuffleDependency[_, _, _], jobId: Int): ShuffleMapStage = {
    ...
    //todo 递归！！！！
    //todo 递归！！！！
    //todo  获取父依赖，记住这个是从缓存中获取。有点绕，但是我推荐拿代码debug跟一下。
    val parents = getOrCreateParentStages(rdd, jobId)

    val id = nextStageId.getAndIncrement()

    logInfo("创建ShuffleMapStage ")
    val stage = new ShuffleMapStage(
      id, rdd, numTasks, parents, jobId, rdd.creationSite, shuffleDep, mapOutputTracker)

    logInfo("更新缓存。。。。 ")

    logInfo("已经成功划分Stage： "+ id )
    ...
    if (!mapOutputTracker.containsShuffle(shuffleDep.shuffleId)) {
      ...
      logInfo("建立任务追踪  。。。   Registering RDD " + rdd.id + " (" + rdd.getCreationSite + ")")
  ...
  private def createResultStage(
    ...
    logInfo("根据jobId 创建 ResultStage  。。。。。。。")


    logInfo("获取所有父stage   ：  getOrCreateParentStages  ")
    val parents = getOrCreateParentStages(rdd, jobId)
    ...
    logInfo("将最后一个stage， 持久化  ： HashMap[Int, Stage] ")
    stageIdToStage(id) = stage

    logInfo("注册 stage  updateJobIdStageIdMaps  ")
    updateJobIdStageIdMaps(jobId, stage)
  ...
  private def getOrCreateParentStages(rdd: RDD[_], firstJobId: Int): List[Stage] = {
    ...
    logInfo("递归循环主体： 根据给定的RDD创建 ShuffleMapStage ， 当前传入的RDD ID : "+ rdd.id  )


    logInfo("根据给定的RDD获取或者创建父stages列表 ，新的stage会根据提供的firstJobId进行创建 ")

    val stage = getShuffleDependencies(rdd)
    logInfo("stage： "+ stage)
    val rs = stage.map {

      shuffleDep => {

        logInfo("创建rdd对应的所有祖先Stage   shuffleDep： "+ shuffleDep)
        val shuffleMapStage =  getOrCreateShuffleMapStage(shuffleDep, firstJobId)
        shuffleMapStage
  ...
  private def getMissingAncestorShuffleDependencies(
    ...
    logInfo("ArrayStack  栈是一种后进先出(LIFO)的数据结构。  ")
    ...
    while (waitingForVisit.nonEmpty) {
      ...
      if (!visited(toVisit)) {
        ...
        val  dep = getShuffleDependencies(toVisit)
        logInfo("dep : "+dep)
        dep.foreach { shuffleDep =>
  ...
  private[scheduler] def getShuffleDependencies(
    ...
    logInfo("这是一个递归方法，获取shuffle依赖的集合 getShuffleDependencies 。。。。")

    logInfo("声明 parents 要返回的父RDD列表。。。。")
    val parents = new HashSet[ShuffleDependency[_, _, _]]

    logInfo("声明 visited 已经访问过的rdd集合。。。 ")
    val visited = new HashSet[RDD[_]]

    logInfo("声明 waitingForVisit 等待访问的rdd集合。。。 ")
    val waitingForVisit = new ArrayStack[RDD[_]]

    logInfo("将待处理的rdd，加载进 waitingForVisit 集合。。。 ")
    waitingForVisit.push(rdd)


    logInfo("循环遍历 waitingForVisit 集合。。。 ")
    while (waitingForVisit.nonEmpty) {


      val toVisit = waitingForVisit.pop()
      logInfo("从待处理rdd集合 waitingForVisit中拿出rdd ==》 toVisit ： " + toVisit)

      if (!visited(toVisit)) {
        logInfo("该toVisit rdd 没有被访问过。。。。 ")
        logInfo("将toVisit 放入到访问队列。。。。 ")
        visited += toVisit

        logInfo("将toVisit RDD 中的所有的rdd进行循环，获取 shuffer  rdd   ")
        val dep = toVisit.dependencies

        dep.foreach {
          case shuffleDep: ShuffleDependency[_, _, _] =>

            logInfo("表示对shuffle阶段输出的依赖性。 请注意，在shuffle的情况下，RDD是瞬态的，因为我们在执行程序端不需要它。 ")

            logInfo("将  shuffleDep 加入到 等待处理的 parents 队列中 。。。  ")
            parents += shuffleDep
          case dependency =>
            logInfo("当前rdd为 shuffle  rdd ， 将rdd 加入到 等待处理的 waitingForVisit 队列中 。。。  ")
  ...
  private def getMissingParentStages(stage: Stage): List[Stage] = {
    logInfo("getMissingParentStages 递归调用visit方法：  " +
    ...
    def visit(rdd: RDD[_]) {
      if (!visited(rdd)) {
        ...
        logInfo("先判断是否有未cache的分区，若全部都被cache了就不用计算parent Stage了。")
        logInfo("遍历rdd的所有依赖，当是宽依赖时获取其对应依赖的宽依赖并判断该stage是否可用。")
        logInfo("判断条件是该stage输出个数是否等于该stage的finalRDD分区数。")
        logInfo("不等于时说明还有未计算的分区，则将该stage加入missing；")
        logInfo("若为窄依赖则继续往上遍历。")
          ...
          for (dep <- rdd.dependencies) {
            dep match {
              case shufDep: ShuffleDependency[_, _, _] =>
                logInfo("这里调用getShuffleMapStage不像在getParentStages时需要划分stage，" +
                  "而是直接根据shufDep.shuffleId获取对应的ShuffleMapStage")
              ...
              case narrowDep: NarrowDependency[_] =>
                logInfo("若stage得状态为available则为未提交stage")
    ...
    while (waitingForVisit.nonEmpty) {
      ...
      logInfo("now waitingForVisit : " + rdd.id )
      visit(rdd)
            ...
            def removeStage(stageId: Int) {
              ...
              for (stage <- stageIdToStage.get(stageId)) {
                if (runningStages.contains(stage)) {
                  logInfo("Removing running stage %d".format(stageId))
                  runningStages -= stage
                }
                ...
                if (waitingStages.contains(stage)) {
                  logInfo("Removing stage %d from waiting set.".format(stageId))
                  waitingStages -= stage
                }
                if (failedStages.contains(stage)) {
                  logInfo("Removing stage %d from failed set.".format(stageId))
                  failedStages -= stage
                }
              }
              // data structures based on StageId
              stageIdToStage -= stageId
              logInfo("After removal of stage %d, remaining stages = %d"
  ...
  def submitJob[T, U](
    ...
    logInfo("Submit an action job to the scheduler......")

    logInfo("Check to make sure we are not launching a task on a partition that does not exist.")
    ...
    logInfo("重点！！！ 申请调用JobSubmitted 方法：  ： " + jobId)
  ...
  def runJob[T, U](
    ...
    logInfo("提交一个action任务给scheduler，并返回一个JobWaiter。。。")
  ...
  private[scheduler] def handleBeginEvent(task: Task[_], taskInfo: TaskInfo) {
    ...
    //todo SparkListenerTaskStart 这个调用的是哪里？？？？
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    logInfo("DAGScheduler#handleJobSubmitted........")
    var finalStage: ResultStage = null
    try {
      logInfo(" New stage creation may throw an exception if, for example, jobs are run on a")
      logInfo(" HadoopRDD whose underlying HDFS files have been deleted.")

      logInfo("创建ResultStage         DAGScheduler#handleJobSubmitted:createResultStage      ........")
      logInfo("Stage划分过程是从最后一个Stage开始往前执行的，最后一个Stage的类型是ResultStage...")
      logInfo("反向驱动，正向提交  ........")

      finalStage = createResultStage(finalRDD, func, partitions, jobId, callSite)
      logInfo("创建finalStage 结束："+ finalStage)
    ...
    logInfo("创建ActiveJob    DAGScheduler#handleJobSubmitted:ActiveJob      ........")
    val job = new ActiveJob(jobId, finalStage, callSite, listener, properties)

    logInfo("清理本地缓存   clearCacheLocs 。。。。。。。")
    clearCacheLocs()
    ...
    logInfo("将提交的任务加入缓存  HashMap[Int, ActiveJob]  ")
    ...
    //TODO 这个不知道调用的哪。。。。
    logInfo("向LiveListenerBus发送Job提交事件")
    listenerBus.post(
      SparkListenerJobStart(job.jobId, jobSubmissionTime, stageInfos, properties))

    logInfo("DAGScheduler#handleJobSubmitted：submitStage提交stage。。。。。。。")
  ...
  private def submitStage(stage: Stage) {
    logInfo("递归提交stage， 但是首先提交第一个stage，即没有父类的stage......")


    val jobId = activeJobForStage(stage)
    logInfo("获取jobId："+ jobId )
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        logInfo("该stage 没有加入等待队列，没有正在运行，没有运行失败。。。。 ")

        logInfo("根据stageid从小到大排序，是因为越前面的stageid越小 ")
        val missing = getMissingParentStages(stage).sortBy(_.id)


        logInfo("RDD: "+ stage.id + "获取顶级 父stage  ：   missing: " + missing)
        ...
        if (missing.isEmpty) {
          ...
          logInfo("这个是核心： 提交尚未执行的stage, 即为stage0，因为他没有父的stage" +
            "也就是说，在这里提交的是stage0 ")
          ...
          logInfo("若有未提交的父Stage，则递归提交父Stage " +
                  " 标记当前stage为waitingStages ，先等待父stage执行完")
          for (parent <- missing) {
            submitStage(parent)
          }

          logInfo("将stage加入等待任务队列  waitingStages ： "+ stage)
          waitingStages += stage

        }

      }else{
        logInfo("该stage 或加已入等待队列，或正在运行，或运行失败： "+ stage )
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    logInfo("对于Shuffle类型的stage，需要判断stage中是否缓存了该结果；)")
    logInfo("对于Result类型的Final Stage，则判断计算Job中该partition是否已经计算完成)")
    logInfo("这么做的原因是，stage中某个task执行失败其他执行成功地时候就需要找出这个失败的task对应要计算的partition而不是要计算所有partition)")


    logInfo(" First figure out the indexes of partition ids to compute. ")
    logInfo(" 找出当前stage的所有分区中，还没计算完分区的stage")
    val partitionsToCompute: Seq[Int] = stage.findMissingPartitions()

    logInfo(" Use the scheduling pool, job group, description, etc. from an ActiveJob associated ")
    logInfo(" with this Stage ")
    logInfo("获取ActionJob中的属性（job group, description 信息）。")
    val properties = jobIdToActiveJob(jobId).properties

    logInfo("将stage加入runningStages")
    runningStages += stage


    logInfo(" SparkListenerStageSubmitted should be posted before testing whether tasks are ")
    logInfo(" serializable. If tasks are not serializable, a SparkListenerStageCompleted event ")
    logInfo(" will be posted, which should always come after a corresponding SparkListenerStageSubmitted ")
    logInfo(" event. ")
    stage match {
    ...
    logInfo("获取Stage的最佳计算位置,即位于哪台机器上。。。。")
    val taskIdToLocations: Map[Int, Seq[TaskLocation]] = try {
      stage match {
        case s: ShuffleMapStage => {
          val rs = partitionsToCompute.map { id => (id, getPreferredLocs(stage.rdd, id))}.toMap
          logInfo("RStage:"+rs )
          rs
        }

        case s: ResultStage =>{
          val RStage = partitionsToCompute.map { id =>
            val p = s.partitions(id)
            (id, getPreferredLocs(stage.rdd, p))
          }.toMap
          logInfo("RStage:"+RStage )
          RStage
        }
    ...
    logInfo(" If there are tasks to execute, record the submission time of the stage. Otherwise, ")
    logInfo(" post the even without the submission time, which indicates that this stage was ")
    logInfo(" skipped. ")
    ...
    logInfo(" TODO: Maybe we can keep the taskBinary in Stage to avoid serializing it multiple times. ")
    logInfo(" Broadcasted binary for the task, used to dispatch tasks to executors. Note that we broadcast ")
    logInfo(" the serialized copy of the RDD and for each task we will deserialize it, which means each ")
    logInfo(" task gets a different copy of the RDD. This provides stronger isolation between tasks that ")
    logInfo(" might modify state of objects referenced in their closures. This is necessary in Hadoop ")
    logInfo(" where the JobConf/Configuration object is not thread-safe. ")
    var taskBinary: Broadcast[Array[Byte]] = null



    logInfo("获取到task最佳位置后，根据不同stage会广播不同序列化后的二进制信息到每个excutor，")
    logInfo( "如果是shuffleMapStage，广播该Stage的FinalRDD和stage的shffleDep；")
    logInfo( "如果是ResultStage，广播Stage的FinalRDD和stage.func。")
    logInfo( "即将task的实际执行逻辑已经序列化到taskBinary中并broadcast到每个executor上。 ")


    var partitions: Array[Partition] = null
    try {
      logInfo(" For ShuffleMapTask, serialize and broadcast (rdd, shuffleDep). ")
      logInfo(" For ResultTask, serialize and broadcast (rdd, func). ")
      var taskBinaryBytes: Array[Byte] = null
      logInfo(" taskBinaryBytes and partitions are both effected by the checkpoint status. We need ")
      logInfo(" this synchronization in case another concurrent job is checkpointing this RDD, so we get a ")
      logInfo(" consistent view of both variables. ")
      ...
      logInfo("广播变量taskBinaryBytes ： "+ taskBinaryBytes)
    ...
    val tasks: Seq[Task[_]] = try {
      ...
      stage match {
        case stage: ShuffleMapStage =>
          ...
          logInfo("为每一个ShuffleMapStage的分区 创建一个 ShuffleMapTask ")
        ...
        case stage: ResultStage =>
          ...
          logInfo("为每一个 ResultStage 的分区 创建一个 ResultTask ")
          partitionsToCompute.map { id =>
    ...
    if (tasks.size > 0) {
      ...
      logInfo("提交任务！！！！    org.apache.spark.scheduler.TaskSchedulerImpl#submitTasks ")
      ...
      logInfo("Because we posted SparkListenerStageSubmitted earlier, we should mark ")
      logInfo("the stage as completed here in case there are no tasks to run ")

      logInfo("将 stage的状态标记为 finished ,并从runningStages 队列中移除  。。。。。。 ")
      markStageAsFinished(stage, None)

      stage match {
        case stage: ShuffleMapStage =>
          logInfo(s"Stage ${stage} is actually done; " +
              s"(available: ${stage.isAvailable}," +
              s"available outputs: ${stage.numAvailableOutputs}," +
              s"partitions: ${stage.numPartitions})")
          markMapStageJobsAsFinished(stage)
        case stage : ResultStage =>
          logInfo(s"Stage ${stage} is actually done; (partitions: ${stage.numPartitions})")
      }
      ...
      case Success =>
        task match {
          ...
          case smt: ShuffleMapTask =>
            ...
            logInfo("ShuffleMapTask finished on " + execId)
      ...
      case FetchFailed(bmAddress, shuffleId, mapId, reduceId, failureMessage) =>
        ...
        if (failedStage.latestInfo.attemptNumber != task.stageAttemptId) {
          ...
          if (runningStages.contains(failedStage)) {
            ...
            logInfo(s"Received fetch failure from $task, but its from $failedStage which is no " +
              s"longer running")
          }
  ...
  private def removeExecutorAndUnregisterOutputs(
    ...
    if (!failedEpoch.contains(execId) || failedEpoch(execId) < currentEpoch) {
      ...
      if (fileLost) {
        ...
        logInfo("Additional executor lost message for %s (epoch %d)".format(execId, currentEpoch))
      }
    }
  }
  ...
  private[scheduler] def handleJobCancellation(jobId: Int, reason: Option[String]) {
    if (!jobIdToStageIds.contains(jobId)) {
      logInfo("Trying to cancel unregistered job " + jobId)
    } else {
  ...
  private def markStageAsFinished(
    ...
    logInfo("Marks a stage as finished and removes it from the list of running stages. ")
  ...
  def getPreferredLocs(rdd: RDD[_], partition: Int): Seq[TaskLocation] = {
    val rs = getPreferredLocsInternal(rdd, partition, new HashSet)
    rs
  }
  ...
  private def getPreferredLocsInternal(
    ...
    logInfo(" getCacheLocs方法中，cacheLocs 维护着RDD的partitions 的 location信息,该信息是TaskLocation的实例。")
    logInfo(" 如果从cacheLocs中获取到partition的location信息直接返回，若获取不到：")
    logInfo(" 如果RDD的存储级别为空返回nil,并填入cacheLocs,否则会通过blocakManagerMaster来获取持有该partition信息的 blockManager")
    logInfo(" 并实例化ExecutorCacheTaskLocation放入cacheLocs中。")



    logInfo("If the partition has already been visited, no need to re-visit.")
    logInfo("This avoids exponential path exploration.  SPARK-695")



    logInfo("当前计算到RDD: id ==》 "+ rdd.id )
    logInfo("如果这个分区已经被访问过，则不需要重新访问。。。。")
    if (!visited.add((rdd, partition))) {
      logInfo("Nil has already been returned for previously visited partitions.")
      return Nil
    }
    ...
    logInfo("如果该分区被缓存过，则返回该分区的位置。。。")
    val cached = getCacheLocs(rdd)(partition)

    if (cached.nonEmpty) {
      return cached
    }
    logInfo("If the RDD has some placement preferences (as is the case for input RDDs), get those")
    logInfo("rdd.preferredLocations，该方法先尝试从checpoint中获取partition信息，" )

    logInfo(  "若未获取到再通过rdd的getPreferredLocations(split)方法获取，" )
    logInfo("不同rdd有不同实现，如HadoopRDD即通过Hadoop InputSplit 来获取当前partition的位置。")
    ...
    logInfo("If the RDD has narrow dependencies, pick the first partition of the first narrow dependency")
    logInfo("that has any placement preferences. Ideally we would choose based on transfer sizes,")
    logInfo("but this will do for now.")

    logInfo("前两者都没有获取到时，则通过递归寻找parentRDD的partition的最佳位置信息。注意：只适用于窄依赖。")
    rdd.dependencies.foreach {
      case n: NarrowDependency[_] =>
        for (inPart <- n.getParents(partition)) {
          val locs = getPreferredLocsInternal(n.rdd, inPart, visited)
          if (locs != Nil) {
            logInfo(rdd.id + "  最佳位置： locs "+ locs.toSet)
            return locs
          }
        }
...
private[scheduler] class DAGSchedulerEventProcessLoop(dagScheduler: DAGScheduler)
  ...
  private def doOnReceive(event: DAGSchedulerEvent): Unit = event match {
    ...
    case JobSubmitted(jobId, rdd, func, partitions, callSite, listener, properties) => {
      ...
      logInfo("job提交方法    DAGScheduler#doOnReceive：JobSubmitted ")
    ...
    case BeginEvent(task, taskInfo) => {
      ...
      logInfo("org.apache.spark.scheduler.DAGScheduler#handleBeginEvent")
