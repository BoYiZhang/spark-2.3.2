...
class CoarseGrainedSchedulerBackend(scheduler: TaskSchedulerImpl, val rpcEnv: RpcEnv)
  ...
  class DriverEndpoint(override val rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    private def makeOffers() {
      ...
      logInfo(" 为所有的executor 提供虚拟的资源。。。。。。")

      // Make sure no executor is killed while some task is launching on it
      logInfo("确保 当task启动的时候 executor 尚未被杀死。。。。。")

      val taskDescs = CoarseGrainedSchedulerBackend.this.synchronized {
        // Filter out executors under killing
        logInfo("过滤出存活的executors。。。。。")
        ...
        logInfo("TaskSchedulerImpl#resourceOffers    调度资源  ")
    ...
    // org.apache.spark.scheduler.cluster.CoarseGrainedSchedulerBackend#launchTasks    Launch tasks returned by a set of resource offers
    private def launchTasks(tasks: Seq[Seq[TaskDescription]]) {
      logInfo("根据分配的资源启动task。。。。。")

      for (task <- tasks.flatten) {

        logInfo("序列化task ：  TaskDescription.encode(task)   。。。。。。。   ")
        val serializedTask = TaskDescription.encode(task)

        logInfo("验证是否超过信息的大小限制：  spark.rpc.message.maxSize  默认128M   ")
        if (serializedTask.limit() >= maxRpcMessageSize) {
          ...
          logInfo("将序列化的task信息发送给executor 。。。   ")

          val executorData = executorDataMap(task.executorId)

          logInfo(" 一个任务，运行几个cpu ：    spark.task.cpus 默认值： 1 ")
          ...
          logInfo("executor 启动task ， 调用：org.apache.spark.executor.Executor#LaunchTask 方法     ")
  ...
  override def start() {
    ...
    logInfo(" (prashant) send conf instead of properties")
  ...
  override def reviveOffers() {
    logInfo("实际会跳转到  DriverEndpoint#receive   的ReviveOffers  。。。。。。。。  ")
