...
/**
  ...
  * 在初始化的时候会调用  addPendingTask(i) 方法初始数据啊！！！！！
  * 重点： def recomputeLocality()
...
private[spark] class TaskSetManager(
  ...
  logInfo("Epoch for " + taskSet + ": " + epoch)
  for (t <- tasks) {
    t.epoch = epoch
  }
    ...
    for (loc <- tasks(index).preferredLocations) {
      loc match {
        ...
        case e: HDFSCacheTaskLocation =>
          // 调用sched（即TaskSchedulerImpl）的getExecutorsAliveOnHost()方法，
          // 获得指定Host上的Alive Executors
          ...
          exe match {
            ...
            case None => logInfo(s"Pending task $index has a cached location at ${e.host} " +
  ...
  /**
    ...
    * 从可以推断的task中找出可推断的task在taskTest中的索引和响应的本地性级别
    *
    ...
    if (!speculatableTasks.isEmpty) {
      ...
      for (index <- speculatableTasks if canRunOnHost(index)) {
        ...
        if (executors.contains(execId)) {
          logInfo("找到了在指定的Executor上推断执行的task。。。。")
      ...
      if (TaskLocality.isAllowed(locality, TaskLocality.NODE_LOCAL)) {
        for (index <- speculatableTasks if canRunOnHost(index)) {
          ...
          if (locations.contains(host)) {
            logInfo("找到了在指定的 本地节点 上推断执行的task。。。。")
            speculatableTasks -= index
      ...
      if (TaskLocality.isAllowed(locality, TaskLocality.NO_PREF)) {
        for (index <- speculatableTasks if canRunOnHost(index)) {
          ...
          if (locations.size == 0) {
            logInfo("对于没有本地性偏好的Task，让他在指定的Executor上执行。。。。")
            speculatableTasks -= index
            return Some((index, TaskLocality.PROCESS_LOCAL))
          }
        }
        ...
        for (rack <- sched.getRackForHost(host)) {
          for (index <- speculatableTasks if canRunOnHost(index)) {
            ...
            if (racks.contains(rack)) {
              logInfo("找到了在指定的  本地机架 上推断执行的task。。。。")
              speculatableTasks -= index
      ...
      if (TaskLocality.isAllowed(locality, TaskLocality.ANY)) {
        for (index <- speculatableTasks if canRunOnHost(index)) {
          logInfo("找到了在 任意节点、机架 上推断执行的task。。。。")
          speculatableTasks -= index
    ...
    : Option[TaskDescription] =
  {
    ...
    if (!isZombie && !offerBlacklisted) {
      ...
      if (maxLocality != TaskLocality.NO_PREF) {
        ...
        //
        //  如果taskset刚刚被提交，taskScheduler开始第一轮对taskset中的task开始提交，
        //  那么当时currentLocalityIndex为0，直接返回可用的最好的本地性；
        //  如果是在以后的提交过程中，那么如果当前的等待时间超过了一个级别，就向后跳一个级别
        //  getAllowedLocalityLevel方法返回的是当前这次调度中，能够容忍的最差的本地性级别，
        //  在后续步骤的搜索中就只搜索本地性比这个级别好的情况
        //  随着时间的推移，撇开maxLocality配置不谈，对于本地性的容忍程度越来越大。
        //  继续返回TaskSetManager.resourceOffer中，获得taskSet能容忍的最差locality后，
        //  与maxLocality比较去较差的locality作为最终的
        //  能容忍的最差locality。



        // todo 获取被允许的Locality，主要是看等待时间  -- 这里即时计算等级的
        allowedLocality = getAllowedLocalityLevel(curTime)

        // 如果allowedLocality大于maxLocality，将maxLocality赋值给allowedLocality
        if (allowedLocality > maxLocality) {
          // We're not allowed to search for farther-away tasks
          //< 临时将允许的locality级别降低到资源允许的最高locality级别   资源超标，做限制
      ...
      //  可以看到，第二段首先调用了函数dequeueTask，如果返回不为空，
      //  说明为指定的worker分配了task；这之后，进行各种信息更新，将taskId加入到runningTask中，
      //  并通知DAGScheduler，最后返回taskDescription。来看看dequeueTask的实现：
      ...
      dequeueTask(execId, host, allowedLocality).map { case ((index, taskLocality, speculative)) =>
        ...
        logInfo("实际调用： org.apache.spark.scheduler.DAGScheduler#handleBeginEvent")
        ...
        logInfo("返回任务信息 TaskDescription : "+ TaskDescription)
        new TaskDescription(
          taskId,
          attemptNum,
          execId,
  ...
  override def checkSpeculatableTasks(minTimeToSpeculation: Int): Boolean = {
    ...
    if (isZombie || numTasks == 1) {
      logInfo("只有一个任务或者 task set  状态是 僵死    没有可以推断的任务。。")
      return false
    }
    var foundTasks = false
    val minFinishedForSpeculation = (SPECULATION_QUANTILE * numTasks).floor.toInt
    logInfo("Checking for speculative tasks: minFinished = " + minFinishedForSpeculation)
    ...
    if (tasksSuccessful >= minFinishedForSpeculation && tasksSuccessful > 0) {
      ...
      // bound based on that.
      logInfo("Task length threshold for speculation: " + threshold)
  ...
  private def computeValidLocalityLevels(): Array[TaskLocality.TaskLocality] = {
    ...
    // 如果pendingTasksForExecutor不为空，
    // 且PROCESS_LOCAL 级别中TaskSetManager等待分配下一个任务的时间不为零，且
    ...
    //pendingTasksForHost 不为空 且 haoostToExecutors 包含
