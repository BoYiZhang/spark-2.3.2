...
abstract class RDD[T: ClassTag](
  ...
  logInfo("每个RDD必须有一个对应的SparkContext ， 否则报错。。。。。")
  ...
  final def dependencies: Seq[Dependency[_]] = {
    logInfo("获取此RDD的依赖项列表，同时考虑RDD是否已检查点...")
  ...
  final def iterator(split: Partition, context: TaskContext): Iterator[T] = {
    if (storageLevel != StorageLevel.NONE) {
      ...
      logInfo("数据未持久化过，尝试从快照中获取或者直接获取&计算。。。。")
  ...
  private[spark] def getOrCompute(partition: Partition, context: TaskContext): Iterator[T] = {
    ...
    logInfo("获取 blockId ， 格式  ： \"rdd_\" + rddId + \"_\" + splitIndex  ")
    val blockId = RDDBlockId(id, partition.index)
    var readCachedBlock = true
    logInfo("This method is called on executors, so we need call SparkEnv.get instead of sc.env.")

    logInfo("这个方法将会请求executors， 所以我们需要申请  SparkEnv.get获取 环境变量")

    SparkEnv.get.blockManager.getOrElseUpdate(blockId, storageLevel, elementClassTag, () => {
      readCachedBlock = false

      logInfo(" 如果rdd 进行checkpoint ， 则对rdd分区进行计算，或者进行读取   ")
   ...
   *
    * 根据指定的numPartitions ， 返回一个新的RDD
    * 结果是一个窄依赖
    * 如果你1000个分区编程100个，不会产生shuffle，相反，100个新的分区，每个里面都会包含10个当前分区
    * 如果一个很大的分区书被请求，将会保持当前的分区数量
    *
    * 回到刚才的分区，如果分区的数量发生激烈的变化，如设置numPartitions = 1，
    * 这可能会造成运行计算的节点比你想象的要少，
    * 为了避免这个情况，可以设置shuffle=true，
    * 那么这会增加shuffle操作。
    *
    * 注意： shuffle = true  你可以合并大量的分区。
    * 当分区数量比较小，比如100的时候，会很有用，一些分区可能会很大
    *
    *
    *
  ...
  def intersection(other: RDD[T]): RDD[T] = withScope {
    this.map(v => (v, null)).cogroup(other.map(f = v => (v, null)))
