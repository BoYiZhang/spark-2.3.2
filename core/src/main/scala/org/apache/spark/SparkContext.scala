...
import org.apache.spark.scheduler.cluster.{CoarseGrainedSchedulerBackend, StandaloneSchedulerBackend, YarnClusterManager}
...
class SparkContext(config: SparkConf) extends Logging {
  ...
  //TODO 我也不知道读的那个配置文件，获取的路径中带null 字符
  config.set("spark.org.apache.hadoop.yarn.server.webproxy.amfilter.AmIpFilter.param.PROXY_URI_BASES","http://bj-rack001-hadoop003:8088")
  ...
  logInfo("加载参数：jars 、files、master、deployMode、appName。。。。。。")
  ...
  logInfo("Used to store a URL for each static file/jar together with the file's local timestamp")
  ...
  logInfo("声明 运行的SparkContext的用户。。。。。 ")
  val sparkUser = Utils.getCurrentUserName()
  logInfo("声明  SchedulerBackend ")
  private[spark] def schedulerBackend: SchedulerBackend = _schedulerBackend
  logInfo("声明  TaskScheduler ")
  private[spark] def taskScheduler: TaskScheduler = _taskScheduler
  private[spark] def taskScheduler_=(ts: TaskScheduler): Unit = {
    _taskScheduler = ts
  }
  logInfo("声明  DAGScheduler ")
  ...
  try {
    ...
    logInfo("创建spark 执行环境变量，这个厉害了，各种manager，各种环境创建啊    (cache, map output tracker, etc)  ")
    ...
    logInfo("设置executor 内存")
    // TODO: Set this only in the Mesos scheduler.
    executorEnvs("SPARK_EXECUTOR_MEMORY") = executorMemory + "m"
    executorEnvs ++= _conf.getExecutorEnv
    executorEnvs("SPARK_USER") = sparkUser

    // We need to register "" before "createTaskScheduler" because Executor will
    // retrieve "HeartbeatReceiver" in the constructor. (SPARK-6640)
    logInfo("在创建createTaskScheduler之前注册 HeartbeatReceiver ")
    _heartbeatReceiver = env.rpcEnv.setupEndpoint(
      HeartbeatReceiver.ENDPOINT_NAME, new HeartbeatReceiver(this))


    logInfo("通过createTaskScheduler方法 创建&启动_schedulerBackend和_taskScheduler ")
    ...
    logInfo("启动TaskScheduler 之后  taskScheduler设置 DAGScheduler")
    ...
    logInfo("初始化blockManager  #initialize 。。。。。。 ")
    ...
    logInfo("启动所有的ExecutorAllocationManager ")
    _executorAllocationManager.foreach(_.start())

    logInfo("创建ContextCleaner ; 清理CleanRDD / CleanShuffle / CleanBroadcast / CleanAccum / CleanCheckpoint ")
    _cleaner =
...
//    _taskScheduler.postStartHook()
  ...
  def textFile(
    ...
    logInfo("这是 hadoopFile .............")
    val rdds = hadoopFile(path, classOf[TextInputFormat], classOf[LongWritable], classOf[Text],
      minPartitions)

     val rdd = rdds.map(pair => pair._2.toString).setName(path)
  ...
  def runJob[T, U: ClassTag](
    ...
    logInfo("SparkContext 当前状态是否关闭 ： " + stopped.get())
    if (stopped.get()) {
    ...
    //递归清理外围类中无用域，降低序列化的开销，防止不必要的不可序列化异常。
    //降低网络io,提高executor的内存效率
    ...
    logInfo("ZHON执行===>   dagScheduler.runJob"  )
...
object SparkContext extends Logging {
  ...
  /**
    * 根据给定的master url 创建一个 task scheduler
  ...
  private def createTaskScheduler(
    ...
    master match {
      ...
      case LOCAL_N_FAILURES_REGEX(threads, maxFailures) =>
        logInfo("LOCAL_N_FAILURES_REGEX module ........")
        def localCpuCount: Int = Runtime.getRuntime.availableProcessors()
      ...
      case LOCAL_CLUSTER_REGEX(numSlaves, coresPerSlave, memoryPerSlave) =>
        logInfo("LOCAL_CLUSTER_REGEX module ........")
      ...
      case masterUrl =>
        logInfo("masterUrl module  ==> YarnClusterManager ........")
        ...
        try {
          logInfo("获取scheduler ==> YarnClusterScheduler  ........")
          val scheduler = cm.createTaskScheduler(sc, masterUrl)
          logInfo("获取backend ==> YarnClusterSchedulerBackend  ........")
          val backend = cm.createSchedulerBackend(sc, masterUrl, scheduler)

          logInfo("YarnClusterManager 执行初始化操作initialize   ........")
  ...
  private def getClusterManager(url: String): Option[ExternalClusterManager] = {
    ...
    val yc = new YarnClusterManager
    yc.canCreate("yarn")
    val greeting: Option[ExternalClusterManager] = Some(yc)
    greeting
  }
}
